def greedy_zeuthen(actions,agent_actions):
    agent_1_actions=[]
    agent_2_actions=[]
    agent_3_actions=[]
            ...
            for i in current_actions:
                if i not in left_actions:
                    ...
                    if len(recent_actions.loc[recent_actions.actions==new_action,'agents'])>0:
                        ...
                        old_agent_worse_index = action_max_index=actions.loc[~actions['actions'].isin(recent_actions.actions),old_agent].idxmax()
                        old_agent_worse = actions.loc[actions.index==old_agent_worse_index,'actions'].values[0]
                        ...
                        risk_point_old = actions.loc[actions.actions==old_agent_worse,old_agent].values[0]
                        old_current = actions.loc[actions.actions==new_action,old_agent].values[0]
                        risk_old = (old_current - risk_point_old)/old_current
                        risk_ended = (new_action_expected - worse_choice_expected)/new_action_expected
                    if len(recent_actions.loc[recent_actions.actions==worse_choice,'agents'])>0:
                        ...
                        worst_choice=actions[~actions.actions.isin(recent_actions.actions)].loc[actions[~actions.actions.isin(recent_actions.actions)][ended_agent].idxmax(),'actions']
                        old_agent_1_worse_index = action_max_index=actions.loc[~actions['actions'].isin(recent_actions.actions),old_agent_1].idxmax()
                        old_agent__1_worse = actions.loc[actions.index==old_agent_1_worse_index,'actions'].values[0]
                        risk_point_old_1 = actions.loc[actions.actions==old_agent__1_worse,old_agent_1].values[0]
                        old_current_1 = actions.loc[actions.actions==new_action,old_agent_1].values[0]
                        risk_old_1 = (old_current_1 - risk_point_old_1)/old_current_1
                        risk_ended = (new_action_expected - worse_choice_expected)/new_action_expected
                    ...
                    elif new_action in recent_actions.actions.values[:] and new_action not in beginning_actions and new_action not in changed_actions and q_2>q_1 and new_point>worse_choice_expected and risk_ended>risk_old:
                        recent_actions.loc[recent_actions.agents==ended_agent,'actions']=new_action
                    ...
                    elif worse_choice in recent_actions.actions.values[:] and worse_choice not in beginning_actions and worse_choice not in changed_actions and q_2_1>q_1_1 and worse_point>worst_choice_expected and risk_ended>risk_old_1:                       
    ...
    e2['utility_2']=u2
    e3['utility_3']=u3
    greedy_zeuthen=pd.concat([e1,e2,e3],axis=1,ignore_index=True)
    greedy_zeuthen.columns=['actions_1','utility_1','actions_2','utility_2','actions_3','utility_3']
    greedy_zeuthen=greedy_zeuthen.fillna(method='ffill')
    greedy_zeuthen['total']=greedy_zeuthen['utility_1'] + greedy_zeuthen['utility_2'] + greedy_zeuthen['utility_3']

    return greedy_zeuthen,changed_actions
